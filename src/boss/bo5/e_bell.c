...
// Donor: src/st/dai/e_bell.c (matched). bo5.h has no local PAL_ enum, but
// the compiled clut immediates match dai.h's PAL_BELL/PAL_BELL_ROPE
// exactly (verified against the raw asm), so they are inlined here.
enum {
    PAL_BELL = 0x75,
    PAL_BELL_ROPE = 0x76,
};
