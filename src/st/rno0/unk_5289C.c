...
// Donor: src/st/e_sealed_door.h (shared by no0/rno2/dai/rnz0/nz0, all
// matched). SEALED_DOOR_PALETTE is 0x259 here (vs NO0's 0x245),
// confirmed via the "sh $v0, 0x16($s1)" store of self->palette in the
// target asm; SEALED_DOOR_CLUT stays PALETTE-1 (0x258), same formula.
#define SEALED_DOOR_PALETTE 0x259
#define SEALED_DOOR_CLUT (SEALED_DOOR_PALETTE - 1)

extern EInit g_EInitCommon;
extern u8 D_us_801833CC[][8];  // g_eBlueDoorUV
extern s16 D_us_801833E4[][8]; // g_eBlueDoorTiles
extern char D_us_80183424[];   // sealed_door_label
...
void EntitySealedDoor(Entity* self) {
    ...
    case 1:
        ...
        if (!(((PLAYER.facingLeft != GetSideToPlayer()) & 1) ^ 1) &&
            ...
            if (!(g_Status.relics[RELIC_JEWEL_OF_OPEN] & RELIC_FLAG_ACTIVE)) {
                ...
                messageBox->ext.messageBox.label = D_us_80183424;
                break;
            }
            prim = &g_PrimBuf[self->primIndex];
    ...
    if (self->animCurFrame) {
        for (i = 0; i < 4; i++) {
            ...
            g_Tilemap.fg[tileIdx] = D_us_801833E4[params][i];
        }
    } else {
        for (i = 0; i < 4; i++) {
            x = self->posX.i.hi;
            ...
            scrollY = y + g_Tilemap.scrollY.i.hi;
            tileIdx = (scrollX >> 4) + (scrollY >> 4) * g_Tilemap.hSize * 0x10;
            g_Tilemap.fg[tileIdx] = D_us_801833E4[params][i + 4];
        }
    }
}
